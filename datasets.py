...
class MnistDataset(ImageDataset):
    ...
    def get_train_set(use_augmentation):
        ...
        train_set = torchvision.datasets.MNIST(
            train=True,
            root=os.path.join(os.path.join(pathlib.Path.home(),
                                                       'open_lth_datasets'), 'mnist'), download=True)
        return MnistDataset(train_set.data, train_set.targets)
    ...
    def get_test_set():
        test_set = torchvision.datasets.MNIST(
            ...
            root=os.path.join(os.path.join(pathlib.Path.home(),
                                                        'open_lth_datasets'), 'mnist'), download=True)
        return MnistDataset(test_set.data, test_set.targets)

    def __init__(self,  examples, labels):
        tensor_transforms = [torchvision.transforms.Normalize(mean=[0.1307], std=[0.3081])]
        super(MnistDataset, self).__init__(examples, labels, [], tensor_transforms)
...
mnist = argparse.Namespace(
    Dataset=MnistDataset,
    DataLoader=DataLoader,
)

# registered_datasets = {'cifar10': cifar10, 'mnist': mnist, 'imagenet': imagenet}
registered_datasets = {'cifar10': cifar10, 'cifar100': cifar100, 'cifar100_coarse': cifar100_coarse, "mnist": mnist,}
